...
class UserManager(BaseUserManager):
    ...
    def _created_user(self,national_code,phone_number, email,password,is_superuser,is_staff,is_active ,**extra_fields):
        """
        ...
        user = self.model(
             ...
             national_code=national_code, email=email, is_staff=is_staff,is_active=True,
            is_superuser=is_superuser, date_joined=now, **extra_fields)
        if not extra_fields.set('no_password'):
            user.set_password(password)

        user.save(using=self._db)
        return user

    def create_user(self, national_code=None, email=None,phone_number=None, password=None, **extra_fields):

        return self._created_user(national_code, email, phone_number, password,False,False, extra_fields)

    def create_superuser(self, national_code, email, phone_number, password, **extra_fields):
        return self._created_user(national_code, email, phone_number, password,True,True, extra_fields)
...
class User(AbstractBaseUser,PermissionsMixin):
    ...
    phone_number = models.BigIntegerField(_('تلفن همراه'),unique=True,null=True,blank=True,validators=[
        validators.RegexValidator(r'^989[0-3,9]\d{8}$',('شماره همراه خود راد وارد کنید.'),)
    ],
    error_messages={ 'unique':_('شماره وارد شده قبلا ثبت شده است!')}
                  )
    #user penal admin
    is_staff= models.BooleanField(_('کاربر پنل مدیریت'),default=False,help_text=_('با فعال کردن این گزینه این کاربر میتواند به پنل مدیریت دسترسی داشته باشد'))
    #activate user in app
    is_active = models.BooleanField(_('فعال'),default=True,help_text=_('فعال بودن این گزینه نشان دهنده آن است که عضویت کار بر در سیستم فعال است.'))
